...
### 3. 自己改善ループ
- ユーザーから修正を受けたら必ずメモリシステム（`~/.claude/projects/*/memory/`）にfeedbackとして記録する
...
- **例外**: 認証・決済・データ変更のコードは必ずユーザーが手動レビューする

---

## タスク管理

1. **まず計画を立てる**：チェック可能な項目として `task.md` に計画を書く
2. **計画を確認する**：実装を開始する前に確認する
3. **進捗を記録する**：完了した項目を随時マークしていく
4. **変更を説明する**：各ステップで高レベルのサマリーを提供する
5. **結果をドキュメント化する**：`task.md` にレビューセクションを追加する
6. **学びを記録する**：修正を受けた後にメモリシステムにfeedbackを記録する
